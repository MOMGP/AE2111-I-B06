    ...
    for i in range(len(span_pos)-1):
        if span_pos[i] >= geometry[2]: #check if double cell
            ...
            for j in range(len(geometry[0])-1):
                full_list.append(("spar", (float(span_pos[i]), geometry[0][j]),(float(span_pos[i+1]), geometry[0][j]), geometry[1][0], 0))
            ...
            if span_pos[i + 1] <= HLD_pos[0] or (span_pos[i] >= HLD_pos[1] and span_pos[i + 1] <= HLD_pos[2]) or span_pos[i] >= HLD_pos[3]:
                full_list.append(("skin", (float(span_pos[i]), geometry[0][1]), (float(span_pos[i + 1]), 1), float(skin_thickness), int(stringers[0]), True))
                full_list.append(("skin", (float(span_pos[i]), geometry[0][1]), (float(span_pos[i + 1]), 1), float(skin_thickness), int(stringers[0]), False))
            ...
            #add skin
            if span_pos[i+1] <= HLD_pos[0] or span_pos[i] >= HLD_pos[1] and span_pos[i+1] <= HLD_pos[2] or span_pos[i] >= HLD_pos[3]:
                full_list.append(("skin", (float(span_pos[i]), geometry[0][2]), (float(span_pos[i+1]), 1),float(skin_thickness) , int(stringers[0]), True))
                full_list.append(("skin", (float(span_pos[i]), geometry[0][2]), (float(span_pos[i+1]), 1),float(skin_thickness) , int(stringers[0]), False))
